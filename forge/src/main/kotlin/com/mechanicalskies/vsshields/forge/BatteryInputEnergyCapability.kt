...
/**
 * Attaches Forge Energy (IEnergyStorage) capability to ShieldBatteryInputBlockEntity.
 * Allows pipes/cables from other mods to insert FE into the battery input.
 */
class BatteryInputEnergyCapability {

    @SubscribeEvent
    fun onAttachCapabilities(event: AttachCapabilitiesEvent<BlockEntity>) {
        val be = event.`object`
        if (be !is ShieldBatteryInputBlockEntity) return

        val storage = BatteryInputEnergyStorage(be)
        val lazy = LazyOptional.of { storage as IEnergyStorage }

        event.addCapability(
            ResourceLocation("vs_shields", "battery_input_energy"),
    ...
    private class BatteryInputEnergyStorage(private val be: ShieldBatteryInputBlockEntity) : IEnergyStorage {
