...
/**
 * Attaches Forge Energy (IEnergyStorage) capability to ShieldGeneratorBlockEntity.
 * Allows pipes/cables from other mods to insert FE into the generator.
 */
class ShieldEnergyCapability {

    @SubscribeEvent
    fun onAttachCapabilities(event: AttachCapabilitiesEvent<BlockEntity>) {
        val be = event.`object`
        if (be !is ShieldGeneratorBlockEntity) return

        val storage = ShieldEnergyStorage(be)
        val lazy = LazyOptional.of { storage as IEnergyStorage }

        event.addCapability(
            ResourceLocation("vs_shields", "energy"),
    ...
    private class ShieldEnergyStorage(private val be: ShieldGeneratorBlockEntity) : IEnergyStorage {
