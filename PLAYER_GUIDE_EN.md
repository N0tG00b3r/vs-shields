...
- While the module is active and powered, the shield becomes a physical wall: unauthorized entities are pushed back at the shield boundary, and foreign ships are repelled by a force-based collision system — ships feel a solid wall without tumbling or rotation lock.
...
A stealth device that renders your ship invisible to other players and radar systems.

**How it works:**
- Place on a VS2 ship and supply FE. Right-click to activate.
- While active, the ship becomes invisible to external players. The cloaking consumes FE continuously.
- **Crew aboard** can still see the ship normally, plus a subtle rainbow shimmer shell indicating the cloak is active.
- **External players** see only a faint, nearly-invisible Predator-like heat distortion where the ship is — a light-bending shimmer with rainbow iridescence.

**Shield interaction:**
- Activating the cloak **suppresses the shield** — the shield turns off and HP drops to 0.
- When the cloak is deactivated, a **10-second cooldown** begins. After the cooldown, the shield auto-restarts from 0 HP and recharges normally.
- The cooldown duration is configurable via `cloak.cloakShieldCooldownTicks` (default: 200 ticks = 10 seconds).

**Combat break:**
- Shooting **from** or **into** a cloaked ship registers a combat hit against the cloak.
- After **3 hits** (configurable via `cloak.cloakBreakHitThreshold`), the cloak **automatically breaks** and the generator toggle is turned off.
- A **30-second recloak cooldown** begins (`cloak.cloakBreakCooldownTicks = 600`). The cloaking generator cannot be reactivated until the cooldown expires.
- All weapon types count: vanilla projectiles, explosions, CBC shells, CGS projectiles, and CGS hitscan weapons.

**Radar compatibility:**
- Ships cloaked by this device are **hidden from Create Radar** — they do not appear on radar displays and do not trigger RWR (Radar Warning Receiver) alerts.
- The Ship Analyzer also cannot detect cloaked ships.
...
Place a **Create rotation shaft** next to a **Shield Generator**, **Shield Battery Input**, **Gravity Field Generator**, **Solid Projection Module**, or **Resonance Beacon**. Rotation speed is converted automatically to FE via mechanical coupling:
- **1 FE/tick per 1 RPM**
...
### Shield Appearance
- **Reaction-Diffusion Labyrinth:** The shield renders as an organic, constantly-shifting labyrinth pattern generated by a Gray-Scott reaction-diffusion simulation (128×128 grid, CPU). Small regions of the pattern are continuously erased and regrown, creating a perpetually restructuring "living" surface. The rendering is heavily optimized — shields far from the camera use fewer visual layers, and the simulation updates are throttled for minimal FPS impact. Compatible with shader mods (Iris/Oculus) — clouds and other post-process effects correctly render behind the shield.
...
- **Inside view:** By default the labyrinth pattern is visible from both sides — crew members on the ship can see it around them. Set `hideShieldBubbleInside: true` in the config to hide it for crew.
- **Cloaked mode:** When the ship is cloaked, the shield bubble turns pink-purple with very low transparency (8% alpha) instead of the normal color.
