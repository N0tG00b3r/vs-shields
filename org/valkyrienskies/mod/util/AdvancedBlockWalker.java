...
public final class AdvancedBlockWalker implements Iterator<AdvancedBlockWalker.BlockPosWithDistance> {
        ...
        for (final Ship ship : VSGameUtilsKt.getAllShips(level)) {
            ...
            if (
                !tmpBox2
                    ...
                    .intersectsAABB(
                        tmpBox
                            .setMin(worldBox.f_82288_, worldBox.f_82289_, worldBox.f_82290_)
                            .setMax(worldBox.f_82291_, worldBox.f_82292_, worldBox.f_82293_)
                    )
            ) {
                continue;
            }
            ...
            ship.getWorldToShip().transformPosition(toVec.set(to.f_82479_, to.f_82480_, to.f_82481_));
    ...
    public BlockPosWithDistance next() {
        ...
        final BlockPos pos = walker.value().m_7949_();
        if (!walker.next()) {
    ...
    private double calcDistance(final BlockPos pos, final Ship ship) {
        final AABB block = new AABB(pos).m_82400_(1e-7);
        ...
        if (ship != null) {
            final Vector3d fromVec = ship.getWorldToShip().transformPosition(new Vector3d(from.f_82479_, from.f_82480_, from.f_82481_));
            final Vector3d toVec = ship.getWorldToShip().transformPosition(new Vector3d(to.f_82479_, to.f_82480_, to.f_82481_));
        ...
        Vec3 point = block.m_82371_(from, to).orElse(null);
        if (point == null) {
            return Double.POSITIVE_INFINITY;
        }
        if (ship != null) {
            final Vector3d pointVec = ship.getShipToWorld().transformPosition(new Vector3d(point.f_82479_, point.f_82480_, point.f_82481_));
    ...
    private record BlockWalkerWithShip(BlockWalker walker, Ship ship) {
        ...
        private static boolean withinShipBorder(final Ship ship, final BlockPos pos) {
            ...
            final int x = pos.m_123341_(), y = pos.m_123342_(), z = pos.m_123343_();
