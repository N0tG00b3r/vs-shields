...
public class MixinRouterMenu {
    ...
    private double ValkyrienSkies$distanceCheck(final Vec3 instance, final Vec3 vec3) {
        final Level level = router.m_58904_();
        final Ship ship = VSGameUtilsKt.getShipManagingPos(level, instance);
        if (ship == null)
            return instance.m_82557_(vec3);

        final Vector3d newInstance = ship.getTransform().getShipToWorld().transformPosition(VectorConversionsMCKt.toJOML(instance));
        return VectorConversionsMCKt.toMinecraft(newInstance).m_82557_(vec3);
    }
}
