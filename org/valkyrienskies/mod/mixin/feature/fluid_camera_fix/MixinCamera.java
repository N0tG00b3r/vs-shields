...
public abstract class MixinCamera {
    ...
    private FluidState getFluidInCamera(final BlockGetter instance, final BlockPos blockPos,
        ...
        isShipWater = false;
        if (fluidState[0].m_76178_() && instance instanceof final Level level) {

            final double origX = this.getPosition().f_82479_;
            final double origY = this.getPosition().f_82480_;
            final double origZ = this.getPosition().f_82481_;

            VSGameUtilsKt.transformToNearbyShipsAndWorld(level, origX, origY, origZ, 1,
                (x, y, z) -> {
                    fluidState[0] = instance.m_8055_(BlockPos.m_274561_(x, y, z))
                        .m_60819_();
    ...
    private float fluidHeightOverride(final FluidState instance, final BlockGetter arg, final BlockPos arg2,
        ...
        if (!instance.m_76178_()) {
            if (isShipWater) {
                if (instance.m_76170_()) {
                    return 1;
                }
            }
        }
