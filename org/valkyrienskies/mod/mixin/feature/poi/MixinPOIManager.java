...
public abstract class MixinPOIManager implements OfLevel {
    ...
    public Stream<PoiRecord> getInSquare(Predicate<PoiType> predicate, BlockPos blockPos, int i, Occupancy occupancy) {
        ...
        final AABB aABB = new AABB(blockPos).m_82400_((double) i + 1);
        Stream<ChunkPos> chunkRange = ChunkPos.m_45596_(new ChunkPos(blockPos), j);
        if (this.valkyrienskies$sLevel instanceof ServerLevel sLevel) {
            for (LoadedServerShip ship : VSGameUtilsKt.getShipObjectWorld(sLevel).getLoadedShips().getIntersecting(
                ...
                chunkRange = Stream.concat(chunkRange, ChunkPos.m_45599_(new ChunkPos(chunkRangeBounds.x(), chunkRangeBounds.y()),
        ...
        return chunkRange.flatMap((chunkPos) -> this.getInChunk(predicate, chunkPos, occupancy)).filter((poiRecord) -> {
            ...
            Vec3 vecPos = VSGameUtilsKt.toWorldCoordinates(valkyrienskies$sLevel, new Vec3(blockPos2.m_123341_(), blockPos2.m_123342_(), blockPos2.m_123343_()));
            return Math.abs(vecPos.m_7096_() - blockPos.m_123341_()) <= i && Math.abs(vecPos.m_7094_() - blockPos.m_123343_()) <= i;
        });
    }
    ...
    private Stream<PoiRecord> onGetInRange(Stream<PoiRecord> instance, Predicate<PoiType> predicate, Operation<Stream<PoiRecord>> original, @Local(argsOnly = true) BlockPos arg, @Local(argsOnly = true) int i) {
        final int k = i * i;
        return instance.filter(poiRecord -> POIChunkSearcher.INSTANCE.getWorldPos(poiRecord, this.valkyrienskies$sLevel).m_82557_(Vec3.m_82528_(arg)) <= (double)k);
    }
