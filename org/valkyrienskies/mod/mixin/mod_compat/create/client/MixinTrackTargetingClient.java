...
public class MixinTrackTargetingClient {
    ...
    private static Translate redirectWithShip(PoseTransformStack instance, Vec3 vec3, Operation<Translate> original, @Local(argsOnly = true) Vec3 camera) {
        ClientShip ship = VSClientGameUtils.getClientShip(lastHovered.m_123341_(), lastHovered.m_123342_(), lastHovered.m_123343_());
        if(ship != null) {
            Vector3d inWorldJOML = ship.getRenderTransform().getShipToWorld().transformPosition(VectorConversionsMCKt.toJOML(Vec3.m_82528_(lastHovered)));
            return instance.translate(inWorldJOML.x, inWorldJOML.y, inWorldJOML.z)
                .translate(-camera.f_82479_, -camera.f_82480_, -camera.f_82481_)
