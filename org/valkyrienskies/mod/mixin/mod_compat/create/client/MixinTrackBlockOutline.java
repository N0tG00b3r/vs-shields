...
public class MixinTrackBlockOutline {
    ...
    private static AABB getBoundsToWorld(BezierConnection connection, Operation<AABB> original) {
        return VSGameUtilsKt.transformAabbToWorld(Minecraft.m_91087_().f_91073_, original.call(connection));
    }
    ...
    private static Vec3 getPositionToWorld(final BezierConnection connection, final double t, final Operation<Vec3> original){
        return VSGameUtilsKt.toWorldCoordinates(Minecraft.m_91087_().f_91073_, original.call(connection, t));
    }
    ...
    private static void wrapSelection(PoseStack instance, double d, double e, double f, Operation<Void> original,
        ...
        ClientShip ship = VSClientGameUtils.getClientShip(hitPos.m_123341_(), hitPos.m_123342_(), hitPos.m_123343_());
        if(ship != null) {
            Vector3d posInWorld = ship.getRenderTransform().getShipToWorld().transformPosition(hitPos.m_123341_(), hitPos.m_123342_(), hitPos.m_123343_(), new Vector3d());
            instance.m_85837_(posInWorld.x - camPos.f_82479_, posInWorld.y - camPos.f_82480_, posInWorld.z - camPos.f_82481_);
            instance.m_252781_(ship.getRenderTransform().getShipToWorld().getNormalizedRotation(new Quaternionf()));
