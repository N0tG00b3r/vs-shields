...
public class ShipEmbeddingManager {
    ...
    private void updateEntitySection(EntitySection<?> section){
        VisualizationManager manager = VisualizationManager.get(Minecraft.m_91087_().f_91073_);
        if (manager == null) return;
        section.m_156845_().forEach(
            entity -> {
    ...
    public synchronized VisualEmbedding getOrCreateEmbedding(ClientShip ship, VisualizationContext ctx){
        ...
        vs$shipBEs.entrySet().removeIf(
            entry -> {
                final VisualizationManager manager = VisualizationManager.get(entry.getKey().m_58904_());
        ...
        BlockPos anchor = BlockPos.m_274446_(VectorConversionsMCKt.toMinecraft(ship.getRenderTransform().getPositionInShip()));
        ...
        vs$shipBEs.entrySet().removeIf(
            entry -> {
                if (entry.getValue() == ship) {
                    VisualizationManager.get(entry.getKey().m_58904_()).blockEntities().queueRemove(entry.getKey());
                    return true;
                } else return false;
            }
        );
    ...
    protected static void setEmbeddingTransform(VisualEmbedding embedding, ClientShip ship, Vec3i anchor,
        ...
        poseMatrix.translate(new Vector3f(-origin.m_123341_(), -origin.m_123342_(), -origin.m_123343_()));
        poseMatrix.translate(ship.getRenderTransform().getShipToWorld().transformPosition(anchor.m_123341_(), anchor.m_123342_(), anchor.m_123343_(), new Vector3d()).get(new Vector3f()));
