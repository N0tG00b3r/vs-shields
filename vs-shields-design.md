...
### 4. Shield Jammer (Глушитель Щитов)

**Свойства:**
- Мощная станция Радиоэлектронной Борьбы (Electronic Warfare)
- Мультиблок 3x3x3: 1 Контроллер (в центре спереди), рамки (`shield_jammer_frame`), входы энергии (`shield_jammer_input`)
- Максимальная ёмкость энергии масштабируется от количества рамок (150k FE за каждую, максимум 3.9M FE)
- Только ОДИН глушитель на корабль
- **Механика:** Сканирует окружающее пространство на наличие других кораблей (`VSGameUtilsKt.getAllShips`). В случае пересечения их AABB с зоной действия глушителя наносит прямой урон генератору вражеского щита (-50,000 FE/t) за счёт глубокого расхода своей энергии.
- **Защита от абуза:** В активной фазе и при полном разряде ("перезагрузка") блокирует приём новой энергии в Контроллер до набора 2,500,000 FE, не позволяя поддерживать его включенным бесконечно.
...
### Щит активен (idle):
- Полупрозрачный эллипсоид вокруг корабля (32×48 segments)
- Гексагональная текстура (honeycomb): яркие рёбра, полупрозрачные ячейки
- Цвет по HP: голубой (>50%) → жёлтый (>25%) → красный
- Медленное вращение сетки (1 оборот / 60 сек)

### Щит под ударом:
- ELECTRIC_SPARK + END_ROD + CRIT + FLASH в точке попадания
- Количество масштабируется по урону (6–40 частиц)
- Эффект появляется **на границе щита**, не на позиции снаряда

### Низкая энергия (FE < 20%):
- Многочастотное мерцание с random dropout
- Сдвиг цвета в оранжево-красный
- Интенсивность нарастает от 20% до 0%

### Щит упал (HP = 0):
- 1-секундная анимация: осколки разлетаются от поверхности эллипсоида
- Фаза 1 (5 тиков): 60→20 частиц/тик наружу
- Фаза 2 (10 тиков): тлеющие искры падают вниз

### Барьерная механика:
- Снаряды уничтожаются **на границе щита** (inflated AABB)
- Дружественный огонь блокируется на границе core AABB
- Снаряды с корабля свободно вылетают наружу
