...
## Механика Тарана (Kinetic Ram)

Контроллер `KineticRamControllerBlockEntity` сканирует 3x3x3 структуру (7 блоков: 1 ядро сзади контроллера + 6 граней) для активации. Лимит: 1 таран на корабль.

### Raycast Block Breaking (Интеграция с VS2)
1. **Точка старта**: Локальный центр контроллера переводится в координаты мира.
2. **Точка конца**: Вектор направления переводится в координаты мира, нормализуется и удлиняется в зависимости от скорости корабля (`Math.max(1.5, velocity * 0.1)`).
3. **Raycast**: `level.clip` с параметром `ClipContext.Block.COLLIDER`.
4. **VS2 Перехват**: VS2 автоматически перехватывает рейкаст, вычисляет пересечение с AABB других кораблей в мире, переводит результат попадания обратно в Локальные координаты целевого корабля.
5. **Разрушение**: Если пробит другой корабль или мир, таран поглощает 5,000 FE для мгновенного слома (`Powered Break`) или теряет 5 физических HP (`Unpowered Break`).

### Перезагрузка (Depletion Cooldown)
Когда HP падает до 0, таран ломает свои 6 компонентов `KineticRamPartBlock` без дропа и уходит в 60-секундный (1200 тиков) кулдаун. В этот момент GUI отображает таймер перезагрузки. После кулдауна HP возвращается на максимум (300).
